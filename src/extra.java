    ...
    private static class ServerThread extends Thread {
        ...
        public void run() {
            try (
                ...
                String EncryptedreceiverId = in.readLine(); // Receive encrypted receiver ID from client
                if (EncryptedreceiverId == null) {
                    System.err.println("Receiver ID is not mentioned.");
                    return;
                }
                ...
                if(b){
                    ...
                    byte[] encryptedReceiverIdBytes = Base64.getDecoder().decode(EncryptedreceiverId);
                    String receiverId = decrypt(encryptedReceiverIdBytes, prvKey); // Decrypt receiver ID

                    System.out.println("Receiver ID : " + receiverId); // Print receiver ID

                    //separating timestamp and message from the decrypted message
                    int index = decryptedMessage.indexOf("]");
                    if (index != -1 && index + 2 < decryptedMessage.length()) { // Check if "]" exists and there's enough room for the message
                        String dateString = decryptedMessage.substring(1, index); // Extract the date substring
                        String msgString = decryptedMessage.substring(index + 2); // Extract the message substring
                    ...
                    String ReEncryptedMessage = encrypt(decryptedMessage, pubKey); // Re-encrypt the Decrypted message with receiver's public key
